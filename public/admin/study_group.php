<?php
    ...
    $upload_dir = __DIR__ . "/study_group/";
    ...
    if (isset($_GET['edit'])) {
        $edit_id = $_GET['edit'];
        $stmt = $conn->prepare("SELECT * FROM study_group WHERE id=?");
        $stmt->bind_param("i", $edit_id);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($row = $result->fetch_assoc()) {
            $edit_group_name = $row['group_name'];
    ...
    if (isset($_POST['add'])) {
        ...
        if (!empty($_FILES['photo']['name'])) {
            ...
            if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
                $photo_path = "study_group/" . $file_name;
            }
        }

        $stmt = $conn->prepare("INSERT INTO study_group (group_name, description, photo) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $group_name, $description, $photo_path);
        $stmt->execute();
    ...
    if (isset($_POST['update'])) {
        ...
        $stmt = $conn->prepare("UPDATE study_group SET group_name=?, description=?, photo=? WHERE id=?");
        $stmt->bind_param("sssi", $group_name, $description, $photo_path, $id);
        $stmt->execute();
        header("Location: study_group.php");
        exit();
    }
    ...
    if (isset($_GET['delete'])) {
        $id = $_GET['delete'];
        $stmt = $conn->prepare("SELECT photo FROM study_group WHERE id=?");
        ...
        $stmt = $conn->prepare("DELETE FROM study_group WHERE id=?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        header("Location: study_group.php");
        exit();
    ...
    $result = $conn->query("SELECT * FROM study_group ORDER BY id ASC");
?>
...
<head>
    ...
    <title>Kelompok Studi | Admin</title>
...
<body class="bg-gray-100 flex">
    <aside class="w-64 bg-blue-900 text-white h-screen p-6">
        ...
        <nav>
            <ul>
                ...
                <li class="mb-4"><a href="study_group.php" class="block py-2 px-4 rounded bg-blue-700">📚 Kelompok Studi</a></li>
    ...
    <main class="flex-1 p-8">
        <div class="bg-white shadow-md p-4 rounded-lg mb-6">
            <h1 class="text-xl font-bold">Kelompok Studi</h1>
        </div>

        <!-- Form Tambah / Edit Data -->
        <form method="POST" enctype="multipart/form-data" class="mb-6">
            <input type="hidden" name="id" value="<?php echo $edit_id; ?>">
            <input type="text" name="group_name" placeholder="Kelompok Studi" required class="border p-2 mr-2" value="<?php echo $edit_group_name; ?>">
        ...
        <table class="w-full bg-white shadow-md rounded-lg overflow-hidden">
            <thead>
                <tr class="bg-blue-500 text-white text-center">
                    ...
                    <th class="px-4 py-2">Kelompok Studi</th>
            ...
            <tbody>
                <?php $no = 1; while ($row = $result->fetch_assoc()): ?>
                    <tr class="border-b text-center">
                        ...
                        <td class="px-4 py-2"><?php echo htmlspecialchars($row['group_name']); ?></td>
                        <td class="px-4 py-2"><?php echo htmlspecialchars(substr($row['description'], 0, 100)); ?>...</td>
                        <td class="px-4 py-2">
