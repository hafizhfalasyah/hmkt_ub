<?php
    ...
    $upload_dir = __DIR__ . "/information/";
    ...
    if (isset($_POST['add'])) {
        ...
        if (!empty($_FILES['photo']['name'])) {
            ...
            if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
                $photo_path = "information/" . $file_name;
            }
        }

        $stmt = $conn->prepare("INSERT INTO information (title, description, photo) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $title, $description, $photo_path);
        $stmt->execute();
    ...
    if (isset($_POST['update'])) {
        ...
        $stmt = $conn->prepare("UPDATE information SET title=?, description=?, photo=? WHERE id=?");
        $stmt->bind_param("sssi", $title, $description, $photo_path, $id);
        $stmt->execute();
        header("Location: information.php");
        exit();
    }
    ...
    if (isset($_GET['delete'])) {
        $id = $_GET['delete'];
        $stmt = $conn->prepare("SELECT photo FROM information WHERE id=?");
...
<body class="bg-gray-100 flex">
    <aside class="w-64 bg-blue-900 text-white h-screen p-6">
        ...
        <nav>
            <ul>
                ...
                <li class="mb-4"><a href="information.php" class="block py-2 px-4 rounded bg-blue-700">ℹ️ Informasi</a></li>
    ...
    <main class="flex-1 p-8">
        ...
        <form method="POST" enctype="multipart/form-data" class="mb-6">
            ...
            <input type="text" name="title" placeholder="Judul" required class="border p-2 mr-2" value="<?php echo $edit_title; ?>">
